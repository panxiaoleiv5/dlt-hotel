...
export class IndexComponent implements OnInit {
  ...
  private getDetailClick(action) {
    ...
    this.clearCache();
    this.action = action;
    console.log('xxxxx',this.http.post(apis.detail, this.action))
    ...
    if (action) {
      ...
      if (this.contain(requestType, "FROM")) {
        ...
        if (action.requestParam && action.requestParam.params && action.requestParam.params.length > 0) {
          ...
          this.mockReqParamsFrom.forEach(item=>{
  ...
  sendTest() {
    ...
    if (this.mockShowBlob) {
      ...
      if (this.contain(this.mockMethod, 'GET')) {
        ...
        this.http.post(this.mockUrl, formData).subscribe(data => this.success(data), error => this.error4sendTest(error));
      } else if (this.contain(this.mockMethod, 'PUT')) {
        console.log(123333)
        this.mockReqParamsFrom.forEach(item=>{
          formData.append(item.name, item.value);
        })
        this.http.put(this.mockUrl, formData).subscribe(data => this.success(data), error => this.error4sendTest(error));
...
//退出登陆
  ...
  postform(){
    console.log('mockReqParamsFrom',this.mockReqParamsFrom)
    let formData = new FormData();
    this.mockReqParamsFrom.forEach(item=>{
      formData.append(item.name, item.value);
    })
