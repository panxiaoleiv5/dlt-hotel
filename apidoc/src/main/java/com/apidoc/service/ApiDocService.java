    ...
    public Detail getDetail(Integer id, String methodUUID) {
        ...
        if (split.length == 2) {
            ...
            if (claszz != null) {
                ...
                if (method != null) {
                    String clazzMapping = SpringUtil.getMapping(claszz) == null?"":SpringUtil.getMapping(claszz);
                    String methodMapping = SpringUtil.getMapping(method) == null?"":SpringUtil.getMapping(method);
                    String mapping = servletContextPath + clazzMapping + methodMapping;//url 映射 mapping 为类上的mapping+方法上的mapping
    ...
    private synchronized void isType(List<ApidocParam> list, Integer actionId, String paramName, Class tclass, Type genType,
        ...
        if (genType instanceof ParameterizedType) {
            ...
            if (Collection.class.isAssignableFrom(tclass)) {//是Collection
                ...
                //保存对象中属性名为data的类型
                System.out.println(parameterArgTypes[0]);
                type4data = parameterArgTypes[0];

                String className = parameterArgTypes[0].getTypeName();
                Integer index = className.indexOf("<");
                try {
                    if (index != -1){
                        class4data = Class.forName(className.substring(0, index)).newInstance().getClass();
                    } else {
                        class4data = Class.forName(className).newInstance().getClass();
                    }
                } catch (Exception e) {
        ...
        if (isMyClass(tclass)) {
            ...
            if (fieldList.size() > 0) {
                for (Field field : fieldList) {
                    ...
                    //针对自定义类型 Result<T> 特殊处理
                    if (class4data != null&&(typeClass.equals(Object.class)||typeClass.equals(List.class))) {
                        typeClass = (Class<?>) class4data;
                        class4data = null;

                        if (type4data instanceof ParameterizedType){
                            genericType = type4data;
                        }
                        type4data = null;
                    ...
                    if (typeClass != tclass) {
                        ...
                        isType(list, actionId, fieldName, typeClass, genericType, tclass, item.getId(), isSelf, isReturn);
                    }
                }
            }
        }//isObject end
